...
<%-- <link rel="stylesheet" href="${root}/static/css/food/read.css" /> --%>
					...
					<div class="eat_2">
						...
						<c:forEach var="couponDto" items="${couponDtoList}" varStatus="status" >
							<div class="cp_mySlides fade">
								...
								<img alt="쿠폰이미지" src="/static/ftp/${couponDto.imageName}" onerror="this.src='${root}/static/css/noimage.jpg'">
									...
									<a href="javascript:void(0)" class="reviewInfoClick" >
										...
										<div class="reviewImg">
											...
											<c:forEach var="imgList" items="${img}">
												<img src="/static/ftp/${imgList}" alt="이미지" onerror="this.src='${root}/static/css/noimage.jpg'"/>
												<!-- 
													
													src="C:/Spring/workspace/image/${foodReviewDto.imageName}"
													src="/img/${imgList}"
													onerror="this.src='${root}/static/css/noimage.jpg'"
												 -->
											</c:forEach>
										</div>							
									</a>
			...
			<div class="img">
				<c:choose>
					<c:when test="${foodDto.foodStatus == 'Y'}">
						<img src="/static/ftp/${imageDto.imageName}" alt="이미지"
							 onerror="this.src='${root}/static/css/noimage.jpg'" />
					</c:when>
					<c:when test="${foodDto.foodStatus != 'N'}">
						<img alt="이미지 검토중" src="${root}/static/css/noimage.jpg'">
					</c:when>
				</c:choose>
			</div>
		...
		success : function(data){
			...
			for(var i=0; i < Object.keys(json.imgList).length;i++){
				if (i == 0) {
					...
					mySlides += "<img style='width: 800px; height: 500px; vertical-align:bottom;' src='${root}/static/ftp/"+json.imgList[i].imageName+"'>";
					mySlides += '</div>';
...
<script src="${root}/static/javascript/review/review.js"></script>
</body>
</html>